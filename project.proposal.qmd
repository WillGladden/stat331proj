...
reword the end of this sentence^


However, there are also instances where countries with very low income per person have high percentages of people with basic sanitation. Below is an example of such a country: what country, be specific
...
full.data |>
  ...
  geom_jitter()
```
It is clear that most data points follow the trend; however, more needs to be investigated in order to understand these outliers.
??? weirdly worded
...
full.data |>
  ...
  geom_point() +
  annotate("text", x = 17500, y = 76.5, label = "2019") +
  annotate("text", x = 15000, y = 65.9, label = "1999") +
  labs(title = "Average Income versus Average Sanitation, 1999-2019",
       subtitle = "Average Percentage of People with Basic Sanitation",
       x = "Average Income per Person",
       y = "",
       color = "Year")
  
```
there is a better way to show this data
different color scheme, no rainbow

Year should be on the x-axis,
some measure of the variables relationship on y
...
summary(data.fit)
```
Upon fitting a simple linear regression model for adjusted income per person and percent of people who have access to basic sanitation, we acquired the following regression equation:
...
## Model Fit
Table of variance? Latex table generator

prop of variability in repsonse values accounted by model (kinda R^2)
